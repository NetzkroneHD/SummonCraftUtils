...
public class JoinCommand extends HitBoxCommand {
    ...
    public void onExecute(HitBoxPlayer hp, String[] args) {
        ...
        ht.getPlayer().getCurrentServer().ifPresentOrElse(
                serverConnection -> hp.getPlayer().createConnectionRequest(serverConnection.getServer()),
                () -> hp.sendMessage(ht.getPrefixAndName()+"§7 is currently on no server.")
        );
    }
