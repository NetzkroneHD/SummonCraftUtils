...
public class AdminCommand extends HitBoxCommand {
    ...
    public AdminCommand(HitBoxUtils hitBoxUtils) {
        ...
        this.subcommands.addAll(Arrays.asList("rl", "sql", "packetlog"));
    }
    ...
    public void onExecute(HitBoxPlayer hp, String[] args) {
        ...
        if (args.length == 1) {
            ...
            } else if(args[0].equalsIgnoreCase("packetlog")) {
                hitBoxUtils.setPrintPacketDebug(!hitBoxUtils.isPrintPacketDebug());
                hp.sendMessage("§7Packet-Debug§8:§7 "+(hitBoxUtils.isPrintPacketDebug() ? "§aenabled" : "§cdisabled")+"§7.");
            } else sendHelp(hp);
